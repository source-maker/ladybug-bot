...
def main():
    ...
    health_status, color = determine_health_status(num_lines_changed, num_commits, num_files_changed)
    ...
    **Health Status: ![{health_status}](https://img.shields.io/badge/status-{health_status}-{color})**
    ...
    if not create_or_update_comment(repo, pr_number, token, comment_body) and health_status == 'red':
        exit(1)
